...
export const HealthManagement: React.FC<HealthManagementProps> = ({ currentScenario, inputUser, onComplete, onMistake }) => {
  ...
  const DashboardFilters = () => <div className="mb-4 space-y-3 rounded-2xl border border-slate-700/70 bg-slate-900/80 p-3">
  ...
  const DashboardPreview = () => <div className={`rounded-2xl border p-4 shadow-2xl transition-colors ${dashboardMode === 'technical' ? 'border-cyan-800 bg-slate-950' : 'border-emerald-300 bg-gradient-to-br from-emerald-50 via-white to-cyan-50 text-slate-900'}`}>
