...
type CaseConfig = {
  ...
  resultStatement: string;
  impactStatement: string;
  territories: { name: string; value: number }[];
};

const CASES: Record<string, CaseConfig> = {
  dengue_sp: {
    city: 'Município de Santa Aurora (SP)', problem: 'crescimento persistente da incidência de dengue',
    ...
    territories: [{ name: 'Norte', value: 515 }, { name: 'Centro', value: 372 }, { name: 'Sul', value: 288 }]
  },
  infant_mortality: {
    city: 'Município de Boa Esperança (MA)', problem: 'mortalidade infantil acima da meta municipal',
    ...
    territories: [{ name: 'Rural', value: 18.6 }, { name: 'Norte', value: 15.1 }, { name: 'Centro', value: 11.9 }]
  },
  tb_aids: {
    city: 'Município de Vale Verde (SP)', problem: 'aumento da proporção de coinfecção tuberculose-HIV',
    ...
    territories: [{ name: 'Centro', value: 15.2 }, { name: 'Leste', value: 12.1 }, { name: 'Oeste', value: 8.8 }]
  },
  asthma_poluicao: {
    city: 'Município de Horizonte Azul (SP)', problem: 'crescimento das internações por asma',
    ...
    territories: [{ name: 'Industrial', value: 39 }, { name: 'Centro', value: 29 }, { name: 'Rural', value: 21 }]
  }
};
...
export const HealthManagement: React.FC<HealthManagementProps> = ({ currentScenario, inputUser, onComplete, onMistake }) => {
  ...
  const currentValue = config.values[currentIndex];
  const maxTerritory = Math.max(...config.territories.map(item => item.value));
  ...
  const linePoints = useMemo(() => {
    const min = Math.min(...config.values); const max = Math.max(...config.values);
    return config.values.map((value, index) => `${22 + index * 66},${104 - ((value - min) / Math.max(1, max - min)) * 72}`).join(' ');
  }, [config]);
  ...
  const validateStage = () => {
    ...
    if (stage === 2) {
      ...
      if (!essentials || chartChoice !== 'line' || comparisonChoice !== 'bar' || denominator !== config.denominator || !viewedCitizen) return fail('Um painel decisório precisa de situação atual, linha de base, meta, qualidade, desigualdade territorial, série temporal e comparação adequada. Confira também a versão cidadã.');
  ...
  const result: HealthManagementResult = {
    ...
    dashboardMode, dashboardCards, interventions: INTERVENTIONS.filter(item => selectedInterventions.includes(item.id)).map(item => item.title),
  ...
  const restartModule = () => {
    setStage(0); setPriority(''); setInterpretation(''); setDashboardCards([]); setChartChoice(''); setComparisonChoice(''); setDenominator(''); setFilterYear(config.years.at(-1) || 2025); setDashboardMode('technical'); setViewedCitizen(false); setClassifications({}); setSelectedInterventions([]); setGutPriority(''); setProcessIndicator(''); setResultStatement(''); setImpactStatement(''); setGoal(''); setMonitoring(''); setPublicMessage(''); setFeedback(null); setMistakes(0); setCompleted(false);
  };
  ...
  const DashboardPreview = () => <div className="rounded-2xl border border-cyan-800 bg-slate-950 p-4 shadow-2xl">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><div className="text-[9px] font-bold uppercase tracking-widest text-cyan-400">Painel municipal • {dashboardMode === 'technical' ? 'modo técnico' : 'modo cidadão'}</div><div className="text-sm font-black text-white">{dashboardMode === 'technical' ? config.indicator : `Como está ${config.problem}`}</div></div><select value={filterYear} onChange={event => setFilterYear(Number(event.target.value))} className="rounded-lg border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-white">{config.years.map(year => <option key={year}>{year}</option>)}</select></div>
    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{dashboardCards.includes('current') && <Metric label="Valor no filtro" value={`${currentValue} ${config.unit}`}/>} {dashboardCards.includes('baseline') && <Metric label="Linha de base" value={config.baseline}/>} {dashboardCards.includes('target') && <Metric label="Meta 2029" value={`${config.targetValue} ${config.unit}`} tone="emerald"/>} {dashboardCards.includes('quality') && <Metric label="Qualidade" value="82% de completude — interpretar com cautela" tone="amber"/>} {dashboardCards.includes('territory') && <Metric label="Maior valor territorial" value={`${config.territories.find(item => item.value === maxTerritory)?.name}: ${maxTerritory}`} tone="amber"/>}</div>
    <div className="mt-3 grid gap-3 lg:grid-cols-2">
      <div className="rounded-xl bg-slate-900 p-3"><div className="mb-1 text-[10px] font-bold text-slate-400">TENDÊNCIA 2021–2025</div>{chartChoice === 'line' ? <svg viewBox="0 0 310 120" className="h-32 w-full"><polyline points={linePoints} fill="none" stroke="#22d3ee" strokeWidth="4"/><line x1="18" x2="292" y1="108" y2="108" stroke="#475569"/>{config.values.map((value, index) => <circle key={index} cx={22 + index * 66} cy={Number(linePoints.split(' ')[index].split(',')[1])} r="4" fill="#a5f3fc"/>)}</svg> : <div className="flex h-32 items-center justify-center text-xs text-slate-600">Escolha o gráfico adequado</div>}</div>
      <div className="rounded-xl bg-slate-900 p-3"><div className="mb-2 text-[10px] font-bold text-slate-400">COMPARAÇÃO TERRITORIAL</div>{comparisonChoice === 'bar' ? <div className="space-y-3">{config.territories.map(item => <div key={item.name}><div className="mb-1 flex justify-between text-[10px] text-slate-300"><span>{item.name}</span><span>{item.value}</span></div><div className="h-3 rounded bg-slate-800"><div className="h-3 rounded bg-gradient-to-r from-violet-500 to-cyan-400" style={{ width: `${Math.max(12, item.value / maxTerritory * 100)}%` }}/></div></div>)}</div> : <div className="flex h-32 items-center justify-center text-xs text-slate-600">Escolha a comparação adequada</div>}</div>
    </div>{dashboardMode === 'citizen' && <div className="mt-3 rounded-xl border border-emerald-800 bg-emerald-950/30 p-3 text-xs leading-relaxed text-emerald-100">Em linguagem simples: há diferenças importantes entre regiões. A equipe vai priorizar onde o problema é maior e atualizar este painel para mostrar se o plano está funcionando.</div>}
  </div>;

  if (completed) return <div className="flex h-full flex-col overflow-y-auto bg-slate-950 text-white"><Header/><div className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center p-4 md:p-8"><div className="rounded-3xl border border-emerald-500/40 bg-gradient-to-br from-emerald-950 via-slate-900 to-cyan-950 p-6 shadow-2xl md:p-9"><ClipboardCheck size={44} className="text-emerald-300"/><p className="mt-4 text-xs font-bold uppercase tracking-[.25em] text-emerald-300">Painel e nota técnica aprovados</p><h2 className="mt-2 text-2xl font-black md:text-4xl">Da evidência à decisão em saúde</h2><p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-300">{inputUser || 'Pesquisador(a)'}, você conectou análise epidemiológica, dashboard, causas, orçamento, cadeia lógica, meta e comunicação pública.</p><div className="mt-5 grid gap-3 md:grid-cols-3"><Metric label="Indicador" value={config.indicator}/><Metric label="Desempenho" value={`${score}%`} tone="emerald"/><Metric label="Produtos" value="Dashboard + nota técnica" tone="emerald"/></div><div className="mt-5 grid gap-3 lg:grid-cols-2"><DashboardPreview/><div className="space-y-3"><div className="rounded-xl bg-white/5 p-4 text-xs leading-relaxed"><strong className="text-cyan-200">Ação</strong><div className="mt-1">{result.interventions.join(' + ')}</div></div><div className="rounded-xl bg-white/5 p-4 text-xs leading-relaxed"><strong className="text-cyan-200">Processo</strong><div className="mt-1">{config.processIndicator}</div></div><div className="rounded-xl bg-white/5 p-4 text-xs leading-relaxed"><strong className="text-cyan-200">Resultado → impacto</strong><div className="mt-1">{config.resultStatement} → {config.impactStatement}</div></div></div></div><div className="mt-6 flex flex-col gap-3 sm:flex-row"><button onClick={() => onComplete(result)} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-500 px-5 py-3 font-bold text-emerald-950 hover:bg-emerald-400"><ShieldCheck size={18}/> Salvar os dois produtos e concluir</button><button onClick={restartModule} className="flex items-center justify-center gap-2 rounded-xl border border-slate-600 px-5 py-3 text-sm font-bold"><RotateCcw size={16}/> Refazer</button></div></div></div></div>;
